...
public class NetOpe implements BaseOpe {
    ...
    public <T extends BaseResBean> void doHttpRequsetWithFile(Context context, String model, final int position, FilesBean data, final onNetProcess<T> onNetProcess, final Class<T> t) {
        onNetProcess.onStart(NET_URL + model, "");
        RequestParams requestParams = new RequestParams(NET_URL + model);
