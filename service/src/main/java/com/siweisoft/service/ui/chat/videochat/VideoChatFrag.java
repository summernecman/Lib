...
public class VideoChatFrag extends BaseUIFrag<VideoChatUIOpe, VideoChatDAOpe> {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        getP().getD().setRoleInfo((RoleInfo) getArguments().getSerializable(ValueConstant.DATA_DATA2));
        ...
        if (Value.userBean.getUsertype() != 0) {
            ...
            LogUtil.E("hh:" + getP().getD().getRoleInfo().toString());
            ...
            //打开音频
            ChatInit.getInstance().getAnyChatSDK().UserSpeakControl(Integer.parseInt(getP().getD().getRoleInfo().getUserID()), 1);
        } else {
            if (AnyChatCoreSDK.GetSDKOptionInt(AnyChatDefine.BRAC_SO_VIDEOSHOW_DRIVERCTRL) == AnyChatDefine.VIDEOSHOW_DRIVER_JAVA) {// 如果是采用Java视频显示，则需要设置Surface的CallBack
                ...
                ChatInit.getInstance().getAnyChatSDK().mVideoHelper.SetVideoUser(index, Integer.parseInt(getP().getD().getRoleInfo().getUserID()));
            }
            ChatInit.getInstance().openLocalCamera(Integer.parseInt(getP().getD().getRoleInfo().getUserID()));
            ChatInit.getInstance().loadRemoveVideo(getP().getU().bind.surfaceviewLocal, Integer.parseInt(getP().getD().getRoleInfo().getUserID()));
            ChatInit.getInstance().getAnyChatSDK().SetRecordSnapShotEvent(new AnyChatRecordEventImp(activity));
            ChatInit.getInstance().startRecordVideo(Value.userBean, getP().getD().getRoleInfo());
        }
    }
    ...
    public void onClickEvent(View v) {
        switch (v.getId()) {
            ...
            case R.id.endCall:
                ...
                CallingCenter.getInstance().VideoCallControl(AnyChatDefine.ANYCHAT_STREAMPLAY_EVENT_FINISH, Integer.parseInt(getP().getD().getRoleInfo().getUserID()), AnyChatDefine.BRAC_ERRORCODE_SESSION_REFUSE, 0, 0, "");
                ChatInit.getInstance().closeRemoveVideo(Integer.parseInt(getP().getD().getRoleInfo().getUserID()));
            ...
            case R.id.btn_speakControl_remove:
                if ((Boolean) getP().getU().bind.btnSpeakControlRemove.getTag(R.id.data)) {
                    ...
                    ChatInit.getInstance().getAnyChatSDK().UserSpeakControl(Integer.parseInt(getP().getD().getRoleInfo().getUserID()), 0);
                } else {
